    ...
    methods (Static)
        function job_ids = submit_sample_subs_tasks_direct(globbing_mat, opts)
            ...
            arguments
                ...
                opts.walltime char {mustBeTextScalar} = '02:00:00'  % Reduced from 24h since we're faster
                opts.starting_index_col {mustBeInteger} = 1
                opts.Ncol {mustBeInteger} = 64
                opts.account_name {mustBeTextScalar} = 'joshua_shimony'
                opts.partition {mustBeTextScalar} = 'tier1_cpu'
        ...
        function write_slurm_script(script_file, opts)
            ...
            % addpath(genpath(''/scratch/jjlee/MATLAB-Drive''));
